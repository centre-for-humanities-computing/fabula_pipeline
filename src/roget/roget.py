...
for fn in glob.glob("roget/heads/head*.txt"):
    with open(fn, "rb") as f:
        lines = f.readlines()
        xml = ["<class>"] + [clean(l) for l in lines] + ["</class>"]
        root = etree.fromstring(
            "".join(xml), parser=etree.XMLParser(encoding="windows-1252")
        )
...
def get_roget(reverse):
    "Added function to just get all of rogetS"
    if reverse == True:
        return reverse_roget
    elif reverse == False:
        return roget


def list_all_categories():
    "Added function to retrieve list of all cateogries"
    cats = list(roget.keys())
    cats_id = [(cat, int(re.findall("\d+", cat)[0])) for cat in cats]
    sorted_cats = [cat_id[0] for cat_id in sorted(cats_id, key=lambda x: x[1])]
